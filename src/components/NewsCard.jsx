...
function NewsCard(props) {
  ...
  return (
    <Card
      className={`${styles.newsCard} ${styles.newsCardColors} h-100 d-flex flex-column justify-content-between align-items-center`}
    >
      {/* La click pe continutul card-ului, suntem redirectati catre pagina cu detalii. */}
      <Link to={`/news/${newsId}`}>
        <Card.Img variant="top" src={imgSrc} />
        <Card.Body>
          <Card.Title className={styles.titlecolor}>{title}</Card.Title>
          <Card.Text className={styles.descriptioncolor}>
            {description}
          </Card.Text>
        </Card.Body>
      </Link>
